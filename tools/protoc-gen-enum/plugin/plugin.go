...
func (b *Builder) generateErrCode(e *protogen.Enum, g *protogen.GeneratedFile) {
	...
	g.P(`return &`, b.importGoxGrpc.Ident("ErrResp"), `{Code: `, b.importGoxErrors.Ident("ErrCode"), `(x), Msg: msg}`)
	g.P("}")
	g.P()
	g.P("func (x ", ccTypeName, ") Wrap(err error) *", b.importGoxGrpc.Ident("ErrResp"), " {")
	g.P(`return &`, b.importGoxGrpc.Ident("ErrResp"), `{Code: `, b.importGoxErrors.Ident("ErrCode"), `(x), Msg: err.Error()}`)
	g.P("}")
	g.P()
